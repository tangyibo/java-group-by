...
/**
 * Array based implementation of the Map interface. This implementation uses the
 * minimum amount of memory to store keys and values. Keys are kept in sorted
 * order, so time for inserts and lookups is O(log2(N)).
 */
public class MinimumMemoryMap<K, V> implements Map<K, V> {
	...
	private int binarySearch(ArrayList<K> keys, K key) {
		...
		if (index >= 0) {
			/*
			 * It is possible though unlikely, that two keys have the same hashCode. Check
			 * that object found really is equal to the key.
			 */
			if (keys.get(index).equals(key))
				return index;

			/*
			 * If previous or following entries have the same hashCode then check if they
			 * equal the key.
			 */
	...
	public Set<Map.Entry<K, V>> entrySet() {
		int size = keys.size();
		Set<Map.Entry<K, V>> entrySet = new LinkedHashSet<Map.Entry<K, V>>(size);
